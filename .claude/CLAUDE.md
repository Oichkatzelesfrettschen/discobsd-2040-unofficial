...
AGENTS.md owns these rules. `.claude/CLAUDE.md` is its byte-identical generated
compatibility copy. Claude Code 2.1.278 discovers that file from repository-root
and subdirectory launches, while a relative import in the discovered file does
not expand from a subdirectory. The canonical policy uses literal references
instead of active imports.
