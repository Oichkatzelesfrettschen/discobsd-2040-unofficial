...
> process window, and sys/arch/rp2040/rp2040/mpu.c programs the MPU to fence
> that window (sys/arch/rp2040/doc/MPU.md), so the body's statements that the
> shipped system configures no hardware isolation describe the survey's
> source revisions and stand corrected for the live port.
...
MPU on each core (datasheet 2.4.1) that the live port programs to fence the user window, 264 KB on-chip SRAM, 2 MB external QSPI flash accessed XIP
...
**CONFIRMED: no MMU, no isolation between processes, whole-process swap; the MPU fences the user window from the kernel (sys/arch/rp2040/doc/MPU.md).**
...
| 11 | Process isolation beyond "trust the compiler" | none currently, on either project | Not required for parity, a research problem only if attempted | Item 3's correction: neither RetroBSD nor DiscoBSD isolates processes today. RP2040 carries an eight-region PMSAv6 MPU on each core (datasheet 2.4.6) that no port programs, and the live RP2040 port programs three of them to close the kernel, the peripherals and the flash to user code (sys/arch/rp2040/doc/MPU.md); isolation between processes stays out of reach because one process is resident at a time in one window, so the remaining problem is a software scheme, and no design for one exists in either codebase. **This is the one item on this list that is a genuine research problem, not an engineering task** -- everything else above has a known solution shape. |
