...
/*
 * ctime(3) promises one fixed-width 26-byte string. The field checks keep
 * each conversion inside its slot, and the bounded formatter makes that
 * invariant survive a later format change.
 */
char *
asctime(const struct tm *time_value)
{
	...
	if (time_value->tm_wday < 0 || time_value->tm_wday >= DAYS_PER_WEEK ||
	    ...
	    time_value->tm_hour < 0 || time_value->tm_hour >= HOURS_PER_DAY ||
