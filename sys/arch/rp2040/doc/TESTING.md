...
| `check-build-failure` | a failed step cannot pass as success: lib/Makefile's install loop stops at the first failed child and its clean loop visits every child and keeps a failure. The child make is a journaling stub that fails on request, and each negative case asserts the stub's own failure sentence, so the intended child is proven reached. The suite fails on the tree before the fix by behavior, not by a missing fixture |
