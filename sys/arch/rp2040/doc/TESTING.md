...
| `check-getty-contracts` | getty's `setflags()`, `splitflags()` and `applyflags()` compiled from the tree with its flag tables and defaults, `ioctl` answered and recorded: the header's indexes agree with the table's names; an empty entry derives parity-free tab-expanding modes, `RAW` for the read mode and echo on, since `rw` and `ec` are inverted capabilities; explicit `np` selects `PASS8` on the final mode only, `op`/`ep`/`ap` select their parity bits with `ap` winning, `ht` and `hf` select `XTABS` and `RTSCTS`, explicit `ec` turns echo off, explicit `rw` selects `CBREAK`, `f0` overrides the derivation; and the split delivers `LPASS8` to `TIOCLSET` and the low half to `TIOCSETP` |
