...
| `check-getty-contracts` | getty's flag derivation and the exact mode-application function from `main.c`, compiled with its flag tables and an ioctl recorder: the header indexes agree with the table; omitted inverted `rw` and `ec` select `RAW` and echo, while explicit `rw` and `ec` select `CBREAK` and disable echo; `np` delivers `LPASS8` after the low mode through ordered `TIOCSETP` and `TIOCLSET` calls; explicit `ep` or `op` replaces the shipped `ap` default; tab, flow-control and explicit `f0` choices retain their meanings. A fixture that drops the local-mode application must fail the same gate. The login half records `TIOCLGET`, a temporary zero-valued `TIOCLSET`, and restoration of the exact inherited local-mode word before the user session starts |
