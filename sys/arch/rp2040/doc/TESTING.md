...
| `check-libc-scanf` | `_doscan` and its float member compiled from the tree at host width, at ILP32 and under the address sanitizer, over the `_IOSTRG` stream `sscanf` builds: every integer base and length modifier, `%i` prefix detection, `%X` identical to `%x`, scansets with ranges, negation, a leading bracket and no carry between directives, `%c` without white-space skipping, `%n` excluded from the count, suppression, literal matching, field widths, and the EOF against matching-failure verdicts. The malformed group feeds a 512-digit run to an unwidthed `%ld` and `%u`. The gate is calibrated against the scanner it replaced: that build aborts under the sanitizer, first on `%X` storing a long through a pointer to `unsigned int`, then on the digit run overrunning a 64-byte staging buffer inside `_innum`'s frame |
