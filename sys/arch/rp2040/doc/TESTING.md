...
| `check-libc-ctime`, `check-libc-ctime-cross` | the exact fixed-width formatter and UTC decomposition compiled as C17 at host, ILP32 and Cortex-M0+ widths: canonical output, one-digit day padding, four-digit year and leap-second boundaries, null input, both sides of every field range, unchanged static output after rejection, epoch and pre-epoch decomposition, leap day, both signed 32-bit time boundaries, an unsuppressed target format-overflow check, and a negative formatter control that accepts invalid fields |
...
| `check-getty-contracts` | getty's flag derivation and the exact mode-application function from `main.c`, compiled with its flag tables and an ordered ioctl trace: the header indexes agree with the table; omitted inverted `rw` and `ec` select `RAW` and echo, while explicit `rw` and `ec` select `CBREAK` and disable echo; `np` delivers `LPASS8` after the low mode through ordered `TIOCSETP` and `TIOCLSET` calls; explicit `ep` or `op` replaces the shipped `ap` default; tab, flow-control and explicit `f0` choices retain their meanings. A fixture that drops the local-mode application must fail the same gate. The login half records `TIOCLGET`, a temporary zero-valued `TIOCLSET`, and restoration of the exact inherited local-mode word. An exact-source order check requires login to clear before authentication commits, restore before environment setup and execute the shell last; missing and prematurely restored fixtures calibrate that check |
