...
| `check-backgammon-contracts` | the shipped `games/backgammon/subs.c` linked against stand-ins for its siblings, driving `readc()` from a pipe and `crterase()` over the erase characters a terminal is set to. `readc()` ends the game on `tchars.t_intrc` and on nothing else, so it hands `CERASE` back for `table.c` and `save.c` to compare, and a rebound interrupt character moves the exit with it; `crterase()` answers yes for backspace and for DEL, which is what decides between `"\010 \010"` and echoing the rubbed-out character. Each case is read from a child process, so a regression in the quit character reports rather than taking the run down with it. Four calibrations fail it: the fixed `'\177'` quit (4 of 17 checks), the fixed `'\003'` quit that 2.11BSD patch 478 proposed (2 of 17), the erase test written as backspace alone (2 of 17) and as DEL alone (1 of 17) |
