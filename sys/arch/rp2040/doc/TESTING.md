...
| `check-getty-contracts` | getty's `setflags()` compiled from the tree with its flag tables: the header's indexes agree with the table's capability names, `np` selects `PASS8` on the final mode only and lands in the local-mode half `main.c` splits for `TIOCLSET`, `ap`, `op` and `ep` select their parity bits with `ap` winning, `ht` and `hf` select `XTABS` and `RTSCTS`, `ec` selects `ECHO`, the read mode defaults to `CBREAK`, and an explicit `f0` overrides the derivation |
