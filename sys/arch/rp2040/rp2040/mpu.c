 ...
 * setting CONTROL.nPRIV before the first user instruction, may reach three
 ...
 * a.out images are loaded into SRAM, so XN stays clear on both. The kernel
 * runs privileged and MPU_CTRL.PRIVDEFENA keeps the default memory map as
 * its background region, so kernel text in XIP flash, kernel data above
 * the window, the peripherals and the PPB stay reachable to the kernel and
 * closed to a process. A user access outside the three regions raises
 * HardFault (datasheet 2.4.6.1), which fault.c delivers as SIGSEGV to the
 ...
 * three do: a map missing a user region would fault the first user
 ...
 * read kernel RAM, kernel flash and SIO and expects each to die of SIGSEGV
 * when machdep.mpu.enable reads 1.
 */
...
static const struct mpu_region mpu_map[] = {
	{ 0x00000000UL, MPU_ROM_LOG2, MPU_AP_RO },
	{ USER_DATA_START, MPU_USER_LOW_LOG2, MPU_AP_RW },
...
mpu_program(u_int n, const struct mpu_region *r)
{
	...
	rasr = r->attrs | MPU_RASR_SIZE(r->log2) | MPU_RASR_ENABLE;
