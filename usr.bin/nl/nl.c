...
/* sbase's text.h struct line, the only piece of it nl.c needs. */
struct line {
	char *data;
	size_t len;
};
...
static char
getlinetype(char *type)
{
	/* sbase also accepts "p<BRE>", matched with POSIX regexec(3);
	 * this libc carries no regcomp/regexec, so that form is out of
	 * scope here and rejected like any other bad type letter. */
	if (!type[0] || !strchr("ant", type[0]))
		usage();
...
main(int argc, char *argv[])
{
	...
	case 'd':
		...
		break;
	case 'f':
		type[0] = getlinetype(EARGF(usage()));
		break;
	case 'b':
		type[1] = getlinetype(EARGF(usage()));
		break;
	case 'h':
		type[2] = getlinetype(EARGF(usage()));
		break;
	case 'i':
		incr = estrtonum(EARGF(usage()), 0, LONG_MAX);
		break;
	case 'l':
		blines = estrtonum(EARGF(usage()), 0, LONG_MAX);
		break;
	case 'n':
	...
	case 's':
		...
		break;
	case 'v':
		startnum = estrtonum(EARGF(usage()), 0, LONG_MAX);
		break;
	case 'w':
