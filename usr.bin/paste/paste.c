...
main(int argc, char *argv[])
{
	...
	/* Populate delimiters: a rune is a byte here (see compat.h), so
	 * the byte string sbase decodes with utfntorunestr() is copied
	 * one Rune per byte instead. */
	delim_rune = ereallocarray(NULL, delim_bytelen + 1, sizeof(*delim_rune));
	for (i = 0; i < delim_bytelen; i++)
		delim_rune[i] = (Rune)delim[i];
	if (!(delim_runelen = delim_bytelen))
		usage();
