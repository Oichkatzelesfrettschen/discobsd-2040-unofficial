...
screenalloc(void)
{
	...
	if ( Realscreen == NULL || Nextscreen == NULL ) {
		windrestore();
		fprintf(stderr,"Unable to allocate the screen buffers!\n");
		exit(1);
	}
}
...
readfile(char *fname, char *fromp, int nochangename)	/* nochangename: if 1, don't change the Filename */
{
	...
	/* Read the whole file. Upstream stopped at CP/M's SUB (0x1A),
	 * which on this system silently truncates any file holding a
	 * control-Z and writes the truncation back on the next :w. */
