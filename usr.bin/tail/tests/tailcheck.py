...
def model(data, opt):
    ...
    while body and body[0].isdigit():
        digits += body[0]; body = body[1:]
    n = int(digits) if digits else -1
    if not fromend and n > 0:
        n -= 1
    bylines = None; bkwds = False
    for c in body:
        if c == "b":
            if n == -1: n = 1
            n <<= 9; bylines = False
        elif c == "c": bylines = False
        elif c == "l": bylines = True
        elif c == "r": bkwds = True; fromend = True; bylines = True
    if n == -1:
...
def main():
    tail = sys.argv[1]; small = "--small" in sys.argv
