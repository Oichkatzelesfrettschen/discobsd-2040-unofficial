 ...
 * Trimmed from sbase's libutil/mode.c: getumask() is unused by the
 * two callers of parsemode() this box carries (mkfifo and uudecode
 * pass their own mask), so only parsemode() is kept.
 */
#include <stdlib.h>
#include <string.h>
#include <sys/stat.h>
